...
import { postToSignInTeacher } from "../../../../../api/teacher";
...
const TeacherSignIn: React.FC<RouteComponentProps> = () => {
  const {
  ...
  const onSubmit: SubmitHandler<SignInFormValues> = async ({ email, pwd }) => {
    try {
      await postToSignInTeacher(email, pwd);
